...
export default function CulturalSafety() {
  return (
    <section
      id="cultural-care"
      className="bg-gradient-to-b from-[#f4fafd] to-white py-14 sm:py-20 px-4 sm:px-6 text-[#2178bd]"
    >
      <div className="max-w-7xl mx-auto space-y-16 sm:space-y-20">

        {/* Acknowledgement */}
        <div className="bg-white rounded-3xl border border-[#d0eaff] shadow p-6 sm:p-10 text-center">
          <span className="text-xs text-[#8cc641] bg-[#f0fbe5] px-3 py-1 rounded-full font-medium inline-block mb-2">
            Acknowledgement of Country
          </span>
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mt-2 mb-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6 px-2">
          {[
            ...
            <div
              key={idx}
              className="bg-white p-5 sm:p-6 border border-[#d0eaff] rounded-2xl shadow-sm hover:shadow-lg transition group"
            >
              <div className="bg-gradient-to-r from-[#2178bd] to-[#8cc641] text-white w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-4 rounded-full flex items-center justify-center text-base sm:text-lg">
                {item.icon}
              </div>
              <h3 className="text-base sm:text-lg font-semibold text-[#2178bd] mb-1">
                {item.title}
              </h3>
        ...
        <div className="text-center px-2">
          <h3 className="text-xl sm:text-2xl font-bold mb-3 text-[#2178bd]">Specialized Cultural Care Services</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 text-left">
            {[
              ...
              <div
                key={idx}
                className="bg-white p-5 sm:p-6 border border-[#d0eaff] rounded-2xl shadow-sm"
              >
                <h4 className="font-bold text-[#2178bd] mb-3 text-sm sm:text-base">{item.title}</h4>
                <ul className="text-sm text-gray-700 list-disc pl-4 sm:pl-5 space-y-1">
                  {item.points.map((p, i) => (
                    <li key={i}>{p}</li>
                  ))}
                </ul>
