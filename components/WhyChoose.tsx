  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white text-center">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl sm:text-4xl font-bold text-[#2178bd] mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          {staffingSolutions.map((item, i) => (
            <div
              key={i}
              className="bg-white border border-[#e6eef5] rounded-2xl p-6 shadow-sm hover:shadow-xl transition duration-300 text-left"
            >
              <div className="flex items-center gap-4 mb-3">
                <div className="bg-[#e3f2fc] p-3 rounded-full">
                  {item.icon}
                </div>
                <h3 className="text-lg font-semibold text-[#2178bd]">
                  {item.title}
                </h3>
              </div>
